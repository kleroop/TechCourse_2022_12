...
InfoArch::InfoArch(QWidget *parent) : QWidget(parent), ui(new Ui::InfoArch)
{
    ...
    connect(ui->addCategoryButton, &QPushButton::clicked, this,
            [this]() { createHandler(CategoryTypes::CATEGORY); });
    connect(ui->addSubCategoryButton, &QPushButton::clicked, this,
            [this]() { createHandler(CategoryTypes::SUBCATEGORY); });
    connect(ui->addTeamButton, &QPushButton::clicked, this,
            [this]() { createHandler(CategoryTypes::TEAM); });
}
...
std::vector<CatButton *> InfoArch::btnWrapper(const std::vector<CustomButton *> &buttons)
{
    ...
    for (CustomButton *btn : buttons) {
        bool isActive = false;
        if (btn->category == this->activeCategory || btn->category == this->activeSubCategory)
            isActive = true;
...
std::vector<CustomButton *> InfoArch::getCustomButtons(const std::vector<ICategory> &categories,
                                                       QPushButton *btnTemplate, bool clickable)
{
    ...
    for (const ICategory &category : categories) {
        ...
        if (clickable) {
            connect(tempButton, &QPushButton::clicked, this,
                    [this, tempButton]() { buttonHandler(tempButton); });
        } else {
...
void InfoArch::fillContainer(QLayout *container, const std::vector<ICategory> &categories,
                             bool clickable, bool replace)
{ // todo maybe move to utils
    std::vector<CatButton *> wrappedButtons =
            btnWrapper(getCustomButtons(categories, buttonTemplate, clickable));
    ...
    addCatDialog->onCreateCall([=](const std::string &name) {
        if (!name.empty()) {
            if (type == CategoryTypes::CATEGORY) {
                this->catTree.categories.insert(catTree.categories.begin(),
                                                Category(name, false, nullptr));
            } else if (type == CategoryTypes::SUBCATEGORY) {
                this->activeCategory->children.insert(
                        activeCategory->children.begin(),
                        SubCategory(name, false, this->activeCategory));
            } else if (type == CategoryTypes::TEAM) {
                this->activeSubCategory->children.insert(
                        activeSubCategory->children.begin(),
                        Team(name, false, this->activeSubCategory));
            }
        }

        api.updateCategories(this->catTree,
                             [=](const CategoriesTreeResponse &resp) { updateAllContainers(); });
    });
}
...
{
    LineDrawer(ui->categoriesFrame->layout(), ui->subCategoriesFrame->layout(),
               this->activeCategory);
...
void InfoArch::LineDrawer(QLayout *leftContainer, QLayout *rightContainer, ICategory *category)
{
    if (category) {
        ...
        if (!rightWidgets.empty() && !leftWidgets.empty()) {
            ...
            painter.drawLine(midX, rightBtns.front().y(), midX,
                             activeCat.y()); // vertical from first right to active

            if (rightBtns.back().y() > activeCat.y())
                painter.drawLine(midX, activeCat.y(), midX,
                                 rightBtns.back().y()); // vertical from last right to active
