...
HandlersFactory::createRequestHandler(const Poco::Net::HTTPServerRequest &request)
{
...
Poco::Net::HTTPRequestHandler *HandlersFactory::getHandler(Handlers handler)
{
    ...
    assert((Handlers::HandlersCount == routes.size())
           && "Switch case for routes doesn't match the number of routes");
